...
//$display("====================================== test_raw START ======================================");
     ...
     $display($time,,"%m MID = %0x", MID,"~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
     $display($time,,"%m seq_base_addr >> addr = saddr = 0x%x", saddr);
    //  $display($time,,"%m depth = %0d", depth);
    //  $display($time,,"%m bsize = %0d", bsize);
    //  $display($time,,"%m bleng = %0d", bleng);
    ...
    for (idy=0; idy<depth; idy=idy+bsize) begin
        ...
        $display($time,,"%m read_task loop 돌자");
    end
...
begin
    ...
    $display($time,,"%m read_task inside");
     ...
     $display($time,,"%m read_task inside 2");
end
endtask
//----------------------------------------------------------------
...
begin
	 ...
	 @(posedge ACLK);
	  ...
	  ARADDR <= #1 addr;
	  ARLEN <= #1 leng-1;
	  ARLOCK <= #1 1'b0; // 0 - normal access , 1 - exclusive access
...
begin
	...
	`ifdef AMBA_AXI_PROT
	 ...
	 $display($time, " %m AWADDR= %x", AWADDR);
     $display($time, " %m AWBURST= %x", AWBURST);
     $display($time, " %m AWLEN= %x AWSIZE = %x", AWLEN,AWSIZE);
	 
	 @(posedge ACLK);
	 while(AWREADY == 1'b0) @(posedge ACLK);
	 
	 AWVALID <= #1 1'b0;
	 //$display($time, " %m AWREADY=0x%x , AWVALID=0x%x", AWREADY,AWVALID);
...
begin
     ...
     for (idx=0; idx<leng; idx=idx+1) begin
		  ...
		  $display($time,,"%m WDATA=0x%08x",WDATA);
...
begin
	...
	BREADY <= #1 1'b1;
    $display($time,,"%m BREADY = %x",BREADY);
    $display($time,,"%m BVALID = %x",BVALID);

	 @(posedge ACLK);
	 while(BVALID == 1'b0) begin 
         //$display($time,,"%m still");
        @(posedge ACLK);
     end

$display($time,,"%m never come?");
...
//                   , bsize //size; // 1 ~ 128 byte in a beat
//                   , bleng //leng; // 1 ~ 16  beats in a burst
//                   , 2'h1     // burst type // INCR
//                   );

//         $display($time,,"%m W for loop ) addr = %x",addr);
//         addr = addr + bsize;
//     end
...
//              if(dataWB[idz] != dataRB[idz]) begin
// 			    error = error + 1 ;
// 				$display($time,,"%m Error A:0x%08x D:0x%08x, but 0x%08x expected",
// 				saddr+idz , dataRB[idz],dataWB[idz]);
// 			 end
// 			 else begin
// 			    $display($time,,"%m OK A:0x%08x D:0x%08x",saddr+idy+idz, dataRB[idz]);
// 			 end
